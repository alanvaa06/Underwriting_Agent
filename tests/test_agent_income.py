...
@pytest.mark.asyncio
async def test_income_node_returns_income_analysis_with_dti(strong_applicant_raw):
    llm = FakeListChatModel(
        responses=['{"summary":"Stable W2 income, $12500/mo","dti":0.304,"qualifies":true}']
    )
    delta = await income_analyst_node(_state(strong_applicant_raw), llm=llm, retriever=None)
    ...
    assert "income" in delta["usage"]


@pytest.mark.asyncio
async def test_income_node_handles_missing_employment_gracefully(strong_applicant_raw):
    ...
    delta = await income_analyst_node(_state(raw), llm=llm, retriever=None)
    assert "income_analysis" in delta
...
@pytest.mark.asyncio
async def test_income_node_includes_employment_and_debts_notes_in_prompt(strong_applicant_raw):
    raw = dict(strong_applicant_raw)
    ...
    await income_analyst_node(state, llm=llm, retriever=None)
